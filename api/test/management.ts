...
function managementTests(): void {
  ...
  const packageDescription: string = "Test package for 1.0.0";
  const packageHash: string = "99fb948da846f4ae552b6bd73ac1e12e4ae3a889159d607997a4aef4f197e7bb"; // resources/blob.zip
  const isTestingMetrics: boolean = !!(process.env.REDIS_HOST && process.env.REDIS_PORT);
  ...
  before((): Promise<void> => {
    ...
    return Promise.resolve()
      ...
      .then((accessKeyId: string): void => {
        ...
        const resourcesDirectory: string = path.join(__dirname, "resources");
        const files: string[] = fs.readdirSync(resourcesDirectory);
  ...
  describe("POST access key", (): void => {
    it("creates new access key for existing account with default expiry", (done): void => {
      const accessKeyRequest: restTypes.AccessKeyRequest = testUtils.makeAccessKeyRequest();
    ...
    describe("Access keys can expire", (): void => {
      let oldAccessKey: storage.AccessKey;
      before(() => {
        oldAccessKey = accessKey;
      });

      it("creates new access key which expires in the specified expiry for existing account", function (done) {
        this.timeout(5000);
  ...
  describe("GET access key", (): void => {
    ...
    it("returns 404 for a missing access key", (done): void => {
      const url = "/accessKeys/fake_access_key_name";
  ...
  describe("PATCH access key", (): void => {
    ...
    it("successfully updates an access key", (done) => {
      const newTtl = 1000 * 60 * 60 * 24 * 5; // 5 days
      const newAccessKey = <restTypes.AccessKeyRequest>{
        friendlyName: "new name",
        ttl: newTtl,
      };

      const oldUrl: string = "/accessKeys/" + accessKey.friendlyName;
      const newUrl: string = "/accessKeys/" + newAccessKey.friendlyName;
    ...
    it("successfully expires an access key", (done) => {
      ...
      const url: string = "/accessKeys/" + accessKey.friendlyName;
    ...
    it("cannot create a new access key", (done) => {
      const newAccessKey = <restTypes.AccessKeyRequest>{ friendlyName: "newAccessKey" };
      const url = `/accessKeys/${newAccessKey.friendlyName}`;
    ...
    it("ignores and does not update name field", (done) => {
      ...
      const oldUrl: string = "/accessKeys/" + accessKey.friendlyName;
      const newUrl: string = "/accessKeys/" + newAccessKey.friendlyName;
      const newKeyUrl: string = "/accessKeys/" + newAccessKey.name;
      PATCH(oldUrl, newAccessKey, () => {
        GET(newUrl, (response: any) => {
          assert.equal(response.accessKey.friendlyName, newAccessKey.friendlyName);
          assert.equal(response.accessKey.description, newAccessKey.friendlyName);
    ...
    it("does not change null fields", (done) => {
      const newAccessKey = <restTypes.AccessKeyRequest>{ friendlyName: null };
      PATCH("/accessKeys/" + accessKey.friendlyName, newAccessKey, () => {
        GET("/accessKeys/" + accessKey.friendlyName, (response: any) => {
          assert.equal(response.accessKey.friendlyName, accessKey.friendlyName);
          assert.equal(response.accessKey.description, accessKey.description);
    ...
    it("returns 400 for empty friendlyName field", (done) => {
      const newAccessKey = <restTypes.AccessKeyRequest>{ friendlyName: "" };
      PATCH(
    ...
    it("returns 400 for invalid ttl field", (done) => {
      const newAccessKey = { ttl: "notanumber" };
      PATCH(
        "/accessKeys/" + accessKey.friendlyName,
        newAccessKey,
        () => {
    ...
    it("returns 409 if duplicate friendlyName provided", (done): void => {
      const newAccessKey = { friendlyName: oldAccessKey.friendlyName };
      PATCH(
        "/accessKeys/" + accessKey.friendlyName,
        newAccessKey,
        () => {
  ...
  describe("DELETE access key", (): void => {
    it("successfully deletes an existing access key by name", (done): void => {
      const accessKeyToDelete: restTypes.AccessKeyRequest = testUtils.makeAccessKeyRequest();
  ...
  describe("DELETE sessions", (): void => {
    ...
    it("returns 404 for a machine name that does not have any sessions associated with it", (done): void => {
      const url = "/sessions/fake_machine_name";
  ...
  describe("Apps and deployment tests", () => {
    ...
    describe("GET apps", () => {
      ...
      it("returns 404 for incorrect account", (done) => {
        const url = "/accounts/fake_account_id/apps";
        GET(url, done, 404);
      });

      it("resolves apps by qualified name", (done) => {
        const duplicateApp: restTypes.App = testUtils.makeRestApp();
        POST("/apps", duplicateApp, () => {
          POST(`/apps/${duplicateApp.name}/transfer/${otherAccount.email}`, /*objToSend*/ {}, () => {
            POST("/apps", duplicateApp, () => {
    ...
    describe("POST apps", () => {
      it("creates app for existing account", function (done) {
        const newApp: restTypes.App = testUtils.makeRestApp();
      ...
      it("creates default deployments and default deployment keys", function (done) {
        ...
        POST(url, newApp, (location: string, responseBody: any): void => {
          assert(responseBody);
          const app: restTypes.App = responseBody.app;
          assert(app);
          assert.equal(app.deployments.length, 2);

          for (let i = 0; i < app.deployments.length; ++i) {
            const deploymentName: string = app.deployments[i];
      ...
      it("can override default deployment creation", function (done) {
        const newApp: restTypes.AppCreationRequest = testUtils.makeRestApp();

        newApp.manuallyProvisionDeployments = true;

        let url = "/apps";

        POST(url, newApp, (): void => {
    ...
    describe("DELETE app", () => {
      it("successfully deletes an existing app", (done) => {
        const url: string = "/apps/" + app.name;
        DELETE(url, () => {
          GET(url, done, 404);
        });
      });
    ...
    describe("PATCH app", () => {
      it("successfully updates an existing app", (done) => {
        const newApp: restTypes.App = testUtils.makeRestApp();
        const oldUrl: string = "/apps/" + app.name;
        const newUrl: string = "/apps/" + newApp.name;
      ...
      it("cannot create a new app", (done) => {
        const newApp: restTypes.App = testUtils.makeRestApp();
        const url = "/apps/desiredAppName";

        PATCH(url, newApp, done, 404);
      });

      it("does not change undefined fields", (done) => {
        const newApp = <restTypes.App>{};
      ...
      it("returns 409 if duplicate name provided", (done): void => {
        ...
        POST("/apps", newApp, (): void => {
          const oldName = newApp.name;
          newApp.name = app.name;
    ...
    describe("GET deployments", () => {
      it("returns deployments for existing app", (done) => {
        const url: string = "/apps/" + app.name + "/deployments";
      ...
      it("returns 404 for missing app", (done) => {
        const url = "/apps/fake_app_name/deployments";
        GET(url, done, 404);
      });
    });
    ...
    describe("POST deployment", () => {
      it("creates new deployment with default deployment key", function (done) {
        const newDeployment: restTypes.Deployment = testUtils.makeRestDeployment();
      ...
      it("returns 404 for a missing app", (done) => {
        const newDeployment = { name: "Some Deployment" };
        const url = "/apps/" + "fake_app_name" + "/deployments";
      ...
      it("requires a name for a deployment", (done) => {
        const newDeployment = {};
        const url = "/apps/" + app.name + "/deployments";
    ...
    describe("GET deployment", () => {
      ...
      it("returns 404 for a missing deployment", (done) => {
        const url: string = "/apps/" + app.name + "/deployments/fake_deployment_name";
        GET(url, done, 404);
      });
    });
    ...
    describe("DELETE deployment", () => {
      it("successfully deletes a deployment for an existing app", (done) => {
        const url: string = "/apps/" + app.name + "/deployments/" + deployment.name;
        DELETE(url, () => {
          GET(url, done, 404);
        });
      });
      ...
      it("returns 404 for a missing app", (done) => {
        const url: string = "/apps/fake_app_name/deployments/" + deployment.name;
        DELETE(url, done, 404);
      });
    });
    ...
    describe("PATCH deployment", () => {
      it("updates existing deployment", (done) => {
        const updatedDeployment: restTypes.Deployment = testUtils.makeRestDeployment();
        const oldUrl: string = "/apps/" + app.name + "/deployments/" + deployment.name;
        const updatedUrl: string = "/apps/" + app.name + "/deployments/" + updatedDeployment.name;
      ...
      it("cannot create a new deployment", (done) => {
        const newDeployment: restTypes.Deployment = testUtils.makeRestDeployment();
        const url: string = "/apps/" + app.name + "/deployments/desiredName";
    ...
    describe("GET packageInfo", () => {
      ...
      it("gets package history for empty deployment", (done) => {
        deployment = testUtils.makeStorageDeployment();
        storage.addDeployment(account.id, app.id, deployment).then(() => {
          const url: string = "/apps/" + app.name + "/deployments/" + deployment.name + "/history";
          GET(url, (response: any) => {
      ...
      if (isTestingMetrics) {
        it("gets metrics for deployment", (done) => {
          const url: string = "/apps/" + app.name + "/deployments/" + deployment.name + "/metrics";
    ...
    describe("DELETE history", () => {
      ...
      before(function () {
        ...
        return storage
          ...
          .then((deploymentId: string) => {
            ...
            const otherAppPackage: storage.Package = testUtils.makePackage();
      ...
      it("returns 403 if app with deployment history being cleared is not owned by user", (done) => {
        const url: string = `/apps/${otherApp.name}/deployments/${otherDeployment.name}/history`;
        DELETE(url, done, 403);
      });
    });

    describe("POST release", () => {
      const identicalPackage: string = getTestResource("blob.zip");
      const differentPackage: string = getTestResource("test.zip");

      it("returns 400 if appVersion is not specified", (done) => {
        const url: string = "/apps/" + app.name + "/deployments/" + deployment.name + "/release";
        const releasePackage: storage.Package = testUtils.makePackage();
        POST(url, { packageInfo: releasePackage }, done, differentPackage, 400);
      });

      it("returns 409 if the latest release is identical", (done) => {
        const url: string = "/apps/" + app.name + "/deployments/" + deployment.name + "/release";
        const releasePackage: storage.Package = testUtils.makePackage("1.0.0");
      ...
      it("returns 409 if release of identical package for similar range", (done) => {
        ...
        POST(
          ...
          { packageInfo: releasePackage },
          () => {
            const releasePackage: storage.Package = testUtils.makePackage("1.x");
            POST(url, { packageInfo: releasePackage }, done, identicalPackage, 409);
          },
          identicalPackage
        );
    ...
    describe("POST promote", () => {
      ...
      it("returns 404 if promoting from an empty deployment", (done) => {
        const url: string = `/apps/${app.name}/deployments/${otherDeployment.name}/promote/${deployment.name}`;
      ...
      it("returns 409 if destination deployment has an unfinished rollout", (done) => {
        ...
        storage
          ...
          .then(() => {
            const url: string = `/apps/${app.name}/deployments/${deployment.name}/promote/${otherDeployment.name}`;
      ...
      it("returns 409 if the promoted package is identical", (done) => {
        const identicalPackage: storage.Package = testUtils.makePackage("1.0.0");
        storage
      ...
      it("returns 409 if promotion of identical package for same range", (done) => {
        const identicalPackage: storage.Package = testUtils.makePackage("1.*");
        storage
          .commitPackage(account.id, app.id, deployment.id, identicalPackage)
          .then(() => storage.commitPackage(account.id, app.id, otherDeployment.id, identicalPackage))
          .then(() => {
      ...
      it("returns 201 and promotes the latest package from the source deployment", (done) => {
        const url: string = "/apps/" + app.name + "/deployments/" + deployment.name + "/release";
        const releasePackage: storage.Package = testUtils.makePackage("1.0.1");
        POST(
          url,
          { packageInfo: releasePackage },
          (location: string, result: any) => {
            const url: string = `/apps/${app.name}/deployments/${deployment.name}/promote/${otherDeployment.name}`;
            const newDescription: string = appPackage.description + " changed";
            const newIsMandatory: boolean = !appPackage.isMandatory;
      ...
      it("returns 201 and nullifies rollout for new release as well as previous disabled release", (done) => {
        const url: string = "/apps/" + app.name + "/deployments/" + deployment.name + "/release";
        const secondAppPackage: storage.Package = testUtils.makePackage("1.0.0");
        secondAppPackage.appVersion = "1.0.0";
        secondAppPackage.description = "disabled package";
        secondAppPackage.isMandatory = true;
        secondAppPackage.isDisabled = true;
        ...
        POST(
          ...
          (location: string, result: any) => {
            const url: string = "/apps/" + app.name + "/deployments/" + otherDeployment.name + "/release";
            const otherPackage: storage.Package = testUtils.makePackage("1.0.2");
                ...
                POST(url, { packageInfo: {} }, () => {
                  storage
                    ...
                    .then((newPackageHistory: storage.Package[]) => {
                      const disabledPackage = newPackageHistory[newPackageHistory.length - 2];
                      const promotedPackage = newPackageHistory[newPackageHistory.length - 1];
      ...
      it("can promote to a deployment with no package", (done) => {
        ...
        const packageToPromote: storage.Package = testUtils.makePackage("1.0.0");
        ...
        POST(
          ...
          (location: string, resultBody: any) => {
            const targetDeployment: storage.Deployment = testUtils.makeStorageDeployment();

            storage.addDeployment(account.id, app.id, targetDeployment).then(() => {
              const url: string = `/apps/${app.name}/deployments/${deployment.name}/promote/${targetDeployment.name}`;
      ...
      it("can promote a disabled release", (done) => {
        ...
        const disabledPackage: storage.Package = testUtils.makePackage("1.0.0");
      ...
      it("can promote a release to a different app version", (done) => {
        ...
        const oldAppVersionPackage: storage.Package = testUtils.makePackage("1.0.0");
    ...
    describe("POST rollback", () => {
      it("returns 404 if nothing to rollback to", (done) => {
        const url: string = "/apps/" + app.name + "/deployments/" + deployment.name + "/rollback";
        POST(url, /*body=*/ {}, done, null, 404);
      });

      it("returns 409 if attempting to rollback to an older app store version", (done) => {
        const url = "/apps/" + app.name + "/deployments/" + deployment.name + "/rollback";
        const secondPackage: storage.Package = testUtils.makePackage();
      ...
      it("returns 404 if rolling back to a label that does not exist", (done) => {
        const url: string = "/apps/" + app.name + "/deployments/" + deployment.name + "/rollback/v0";
        const secondPackage: storage.Package = testUtils.makePackage();
      ...
      it("returns 409 if rolling back to a package that is already the latest", (done) => {
        const url: string = "/apps/" + app.name + "/deployments/" + deployment.name + "/rollback/v2";
        const secondPackage: storage.Package = testUtils.makePackage();
        secondPackage.appVersion = "1.0.0";
        deployment.package = secondPackage;
        storage
      ...
      it("returns 409 if rolling back to a label corresponding to a different app version", (done) => {
        const url: string = "/apps/" + app.name + "/deployments/" + deployment.name + "/rollback/v1";
        const secondPackage: storage.Package = testUtils.makePackage();
        secondPackage.appVersion = "1.0.1";
        const thirdPackage = <storage.Package>JSON.parse(JSON.stringify(secondPackage));
      ...
      it("rolls back to previous package", (done) => {
        ...
        storage
          ...
          .then(() => {
            POST(url, /*body=*/ {}, () => {
              GET("/apps/" + app.name + "/deployments/" + deployment.name, (response: any) => {
                const restPackage: restTypes.Package = response.deployment.package;
      ...
      it("can rollback to disabled release", (done) => {
        ...
        const thirdPackage: storage.Package = testUtils.makePackage();
          ...
          .then(() => {
            POST(url, /*objToSend*/ {}, () => {
              GET("/apps/" + app.name + "/deployments/" + deployment.name + "/history", (response: any) => {
                const packageHistory: restTypes.Package[] = response.history;
    ...
    describe("GET collaborators", () => {
      ...
      it("succeeds as owner", (done) => {
        GET("/apps/" + app.name + "/collaborators", (response: any) => {
          const collaboratorMap = <restTypes.CollaboratorMap>response.collaborators;
      ...
      it("succeeds as collaborator", (done) => {
        let otherApp = testUtils.makeStorageApp();
        storage
          .addApp(otherAccount.id, otherApp)
          .then((storageApp) => {
            otherApp = storageApp;
    ...
    describe("DELETE collaborator", () => {
      ...
      describe("as collaborator", () => {
        ...
        it("returns 403 if attempting to delete another collaborator", (done) => {
          const thirdAccount = testUtils.makeAccount();
          storage
    ...
    describe("returns 403 when collaborator tries to", () => {
      ...
      it("update app", (done) => {
        const patchedApp: restTypes.App = { name: "dummy_name" };
      ...
      it("update deployment", (done) => {
        const patchedDeployment: restTypes.Deployment = { name: "dummy_name" };
  ...
  function POST(
    ...
    let newRequest: superagent.Request<any> = request(server || serverUrl)
      .post(url)
